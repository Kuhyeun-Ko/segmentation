...
class SegListMS(torch.utils.data.Dataset):
    ...
    def read_lists(self):
        ...
        label_path = join(self.list_dir, self.phase + '_labels.txt')
        assert exists(image_path)
        self.image_list = [line.strip() for line in open(image_path, 'r')] [:400]
        if exists(label_path):
            self.label_list = [line.strip() for line in open(label_path, 'r')] [:400]
            assert len(self.image_list) == len(self.label_list)
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        # print("output.shape : ", model(input_var).shape)
...
def train_seg(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, writer=writer,
              eval_score=accuracy)
...
def parse_args():
    ...
    parser.add_argument('--checkpoints', default='drn', type=str)
    parser.add_argument('--log', default='log/drn/', type=str)
